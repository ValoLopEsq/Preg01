...
public class ValidarUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                String user, pass;
                user = request.getParameter("puser");
                pass = request.getParameter("ppass");
             
                AlumnowebJpaController usuDAO = new AlumnowebJpaController();
                Alumnoweb u = usuDAO.validar(new Alumnoweb(0, user, pass));

                if (u != null) {
                    out.print("{\"valid\": true}");
                } else {
                    out.print("{\"valid\": false, \"error\": \"Usuario o contraseña incorrectos.\"}");
                }
            } catch (Exception e) {
                e.printStackTrace(); // lo ves en consola
                out.print("{\"valid\": false, \"error\": \"Error en el servidor: " + e.getMessage() + "\"}");
            }
            out.flush();
        }
    }
